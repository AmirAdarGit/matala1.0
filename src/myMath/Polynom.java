...
public class Polynom implements Polynom_able{
	...
	public double area(double x0, double x1, double eps) {	
		...
		for(double i=x0;i<x1;i=i+eps)
		{
			if(this.f(i+x0)>0)
			{
				sum+=eps*Math.abs(f(i+x0));
			}
		}
		return Math.abs(sum);
	}
	/**
	 * this finction give the area between the graph and X, in the positive sise of X
	 * @param x0 represent the start range
	 * @param x1 represent the end range
	 * @param eps represent the accuracy 
	 * @return the area (integral)
	 */
public double area_graph(double x0,double x1,double eps)
	{
		double area=0;
		for(double i=x0;i<x1;i=i+eps)
		{
...
/**
 * this finction print the polynom on the screen
 * @param x0 represent the start range
 * @param x1 represent the end range
 * @param eps represent the accuracy of the points at the polynom
 */
	public void graph(double start, double end, double eps) throws IOException
	{
		...
		BitmapEncoder.saveBitmapWithDPI(chart, "./Sample_Chart_300_DPI", BitmapFormat.PNG, 400);

		
		
		Polynom deriv = new Polynom(this);
		deriv.derivative();
		int count_max_min=1;
		for(int i=0;i<dis;i++)
		{
			if(deriv.f(i*eps+start)*deriv.f(i*eps+start+eps)<0)
			{
				String x =new DecimalFormat("0.00").format(i*eps+start);
				String y =	new DecimalFormat("0.00").format((f(i*eps+start)));
				double temp_arr_x[]= {i*eps+start};
				double temp_arr_y[]= {(f(i*eps+start))};
				String s = Integer.toString(count_max_min);	 
				chart.addSeries("peak point " + s+": [" + x+"],["+y+"]" ,temp_arr_x, temp_arr_y);
				count_max_min++;
			}
		}
	}
